...
@section('title', 'Novo Item P&I Clubs - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Novo Item - {{ $piClub->title }}</h2>
            <p class="text-on-surface-variant font-medium">Cadastre um item com ou sem link.</p>
        </div>
        <a href="{{ route('admin.pi-clubs.items.index', $piClub) }}" class="flex items-center gap-2 bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-bold hover:bg-slate-200 transition-colors shadow-sm">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full max-w-none">
        <form action="{{ route('admin.pi-clubs.items.store', $piClub) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="flex items-center justify-between gap-6 bg-surface-container-low px-5 py-4 rounded-xl border border-slate-100">
                <div>
                    ...
                    <div class="text-xs text-on-surface-variant">Exibir este item na página P&amp;I Clubs.</div>
                </div>
