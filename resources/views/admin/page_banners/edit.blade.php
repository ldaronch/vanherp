...
@section('title', 'Editar Banner de Página - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Editar Banner de Página</h2>
            <p class="text-on-surface-variant font-medium">Atualize a imagem e a página.</p>
        </div>
        <a href="{{ route('admin.page-banners.index') }}" class="flex items-center gap-2 bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-bold hover:bg-slate-200 transition-colors shadow-sm">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full max-w-none">
        <form action="{{ route('admin.page-banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="page" class="block text-sm font-bold text-on-surface-variant mb-2">Página</label>
                <select name="page" id="page" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all" required>
                    @foreach($pages as $key => $label)
                        <option value="{{ $key }}" {{ old('page', $banner->page) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('page') <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="image" class="block text-sm font-bold text-on-surface-variant mb-2">Imagem (deixe em branco para manter)</label>
            ...
            <div class="flex items-center justify-between gap-6 bg-surface-container-low px-5 py-4 rounded-xl border border-slate-100">
                <div>
                    ...
                    <div class="text-xs text-on-surface-variant">Exibir este banner no topo da página.</div>
                </div>
