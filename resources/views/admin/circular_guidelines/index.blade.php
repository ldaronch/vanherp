...
@section('title', 'Circulars & Guidelines (Página) - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Circulars &amp; Guidelines</h2>
            <p class="text-on-surface-variant font-medium">Gerencie seções e itens da página.</p>
        </div>
        <a href="{{ route('admin.circular-guidelines.create') }}" class="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold hover:brightness-110 transition-colors shadow-lg">
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($sections as $section)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-on-surface-variant">{{ $section->items_count }}</td>
                        <td class="px-6 py-4 text-on-surface-variant">{{ $section->sort_order }}</td>
                        <td class="px-6 py-4">
                            <form action="{{ route('admin.circular-guidelines.toggle', $section) }}" method="POST" class="inline-flex items-center">
                                @csrf
                                @method('PATCH')
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('admin.circular-guidelines.items.index', $section) }}" class="inline-flex items-center p-2 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="Itens">
                                <span class="material-symbols-outlined text-sm">list</span>
                            </a>
                            <a href="{{ route('admin.circular-guidelines.edit', $section) }}" class="inline-flex items-center p-2 text-primary hover:bg-primary/10 rounded-lg transition-colors">
                                <span class="material-symbols-outlined text-sm">edit</span>
                            </a>
                            <form action="{{ route('admin.circular-guidelines.destroy', $section) }}" method="POST" class="inline-block" onsubmit="return confirm('Excluir seção?')">
                                @csrf
                                @method('DELETE')
