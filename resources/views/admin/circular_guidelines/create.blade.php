...
@section('title', 'Nova Seção (Página Circulars & Guidelines) - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Nova Seção</h2>
            <p class="text-on-surface-variant font-medium">Cadastre o título e a observação.</p>
        </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full flex-1 flex flex-col">
        <form action="{{ route('admin.circular-guidelines.store') }}" method="POST" class="flex flex-col gap-6 flex-1">
            @csrf
            ...
            <div>
                ...
                <textarea name="note" id="note" rows="6" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all">{{ old('note') }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex items-center justify-between gap-6 bg-surface-container-low px-5 py-4 rounded-xl border border-slate-100">
                    ...
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="show_note" value="1" class="sr-only peer" {{ old('show_note', false) ? 'checked' : '' }}>
                        <div class="relative w-12 h-7 bg-slate-300 rounded-full peer peer-checked:bg-primary transition-colors">
                            <div class="absolute top-1 left-1 w-5 h-5 bg-white rounded-full transition-transform peer-checked:translate-x-5"></div>
                        </div>
                    </label>
