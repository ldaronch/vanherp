...
@section('title', 'Editar P&I Clubs - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Editar Seção P&amp;I Clubs</h2>
            <p class="text-on-surface-variant font-medium">Atualize o título, texto e status.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.pi-clubs.items.index', $section) }}" class="flex items-center gap-2 bg-slate-100 text-slate-700 px-6 py-3 rounded-xl font-bold hover:bg-slate-200 transition-colors shadow-sm">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full max-w-none">
        <form action="{{ route('admin.pi-clubs.update', $section) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="text" id="text" rows="8" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all" placeholder="Texto opcional exibido acima da lista.">{{ old('text', $section->text) }}</textarea>
