...
@section('title', 'Nova Seção P&I Clubs - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Nova Seção P&amp;I Clubs</h2>
            <p class="text-on-surface-variant font-medium">Cadastre o título, texto e status.</p>
        </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full max-w-none">
        <form action="{{ route('admin.pi-clubs.store') }}" method="POST" class="space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    <label for="title" class="block text-sm font-bold text-on-surface-variant mb-2">Título 2</label>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" min="0" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all" value="{{ old('sort_order', 0) }}">
                </div>
            </div>

            <div>
                <label for="text" class="block text-sm font-bold text-on-surface-variant mb-2">Texto da Seção</label>
                <textarea name="text" id="text" rows="8" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all" placeholder="Texto opcional exibido acima da lista.">{{ old('text') }}</textarea>
            ...
            <div class="flex items-center justify-between gap-6 bg-surface-container-low px-5 py-4 rounded-xl border border-slate-100">
                <div>
                    ...
                    <div class="text-xs text-on-surface-variant">Exibir esta seção na página P&amp;I Clubs.</div>
                </div>
