...
@section('title', 'Itens (Circulars & Guidelines) - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Itens - {{ $section->title }}</h2>
            <p class="text-on-surface-variant font-medium">Gerencie itens com ou sem link.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.circular-guidelines.items.create', $section) }}" class="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold hover:brightness-110 transition-colors shadow-lg">
                ...
                @forelse($items as $item)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('admin.circular-guidelines.items.edit', [$section, $item]) }}" class="inline-flex items-center p-2 text-primary hover:bg-primary/10 rounded-lg transition-colors">
                                <span class="material-symbols-outlined text-sm">edit</span>
                            </a>
                            <form action="{{ route('admin.circular-guidelines.items.destroy', [$section, $item]) }}" method="POST" class="inline-block" onsubmit="return confirm('Excluir item?')">
                                @csrf
                                @method('DELETE')
