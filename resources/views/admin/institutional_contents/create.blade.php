...
@section('title', 'Novo - '.$sectionLabel.' - Painel Administrativo')

@section('content')
    <header class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-[2.75rem] font-bold tracking-tight text-on-surface leading-tight mb-2">Novo - {{ $sectionLabel }}</h2>
            <p class="text-on-surface-variant font-medium">Cadastre título, texto, imagem, ordem e status.</p>
        </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-8 border border-slate-100 w-full max-w-none">
        <form action="{{ route('admin.institutional-contents.store', $section) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            @if($section === 'our_services')
                <div>
                    ...
                    <textarea name="items" id="items" rows="6" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all">{{ old('items') }}</textarea>
            ...
            @if($section === 'our_history')
                <div>
                    ...
                    <input type="text" name="image_caption" id="image_caption" class="w-full px-4 py-3 bg-surface-container-low border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all" value="{{ old('image_caption') }}">
